...
//						System.out.println("sharedpoint: "+Arrays.toString(corrdinatelist.get(nextPoi)));
        ...
        for(int pointer=0;pointer<(1<<(numOfPois*2));++pointer)//2^3,选择哪边的交点
        {
            ...
            if(max!=0 && max<min)
            {
                ...
                System.out.println("res: "+min+" "+midPointX/numOfPois+" "+midPointY/numOfPois);
            }
            if(max!=0 && (max<scale*scale || max==min))
            {
                System.out.println("possible: dist:"+max+" pos: "+midPointX/numOfPois+" "+midPointY/numOfPois);
            }
        }

        Double[] midPoint= {bestmidPointX/numOfPois, bestmidPointY/numOfPois};
        System.out.println("Answer: dist:"+min+" pos: "+Arrays.toString(midPoint));
